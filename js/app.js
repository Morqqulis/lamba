(() => {
    ...
    function initSliders() {
        if (document.querySelector(".seven__slider")) new core(".seven__slider", {
    ...
    function showContent() {
        const showMe = document.querySelectorAll(".seven__slide-link");
        showMe.forEach((item => {
            item.addEventListener("click", addClass);
            function addClass() {
                if (item.previousElementSibling.classList.contains("seven__slide-text")) {
                    item.previousElementSibling.classList.add("seven__slide-text_show");
                    item.style.cssText = "visibility: hidden;";
                }
            }
        }));
    }
    document.querySelectorAll(".seven__slide-link") ? showContent() : null;
    let map;
    function initMap() {
        map = new google.maps.Map(document.getElementById("map"), {
            center: {
                lat: 37.23148301168463,
                lng: -8.628194921279588
            },
            zoom: 13.2,
            styles: [ {
